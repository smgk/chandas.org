...
test("the web shell has no external runtime asset dependencies", () => {
    ...
    assert.match(html, /custom_meter\.js/);
...
test("service worker keeps English outside the core shell and runtime-caches it", () => {
    ...
    assert.match(
        read("app.js"),
        /english_forms\.json\?v=\$\{ENGLISH_FORM_ASSET_VERSION\}/
    );
    assert.doesNotMatch(
        read("index.html"),
